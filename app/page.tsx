...
export default function HomePage() {
  ...
  return (
    <div className="w-full bg-slate-950 rounded-3xl p-6 md:p-8 border border-white/10 shadow-2xl relative overflow-hidden" style={{ color: '#ffffff !important' } as any}>
        ...
        {activeTab === 'search' && (
          <div className="space-y-6">
            <div className="text-center max-w-xl mx-auto">
              ...
              <p className="text-sm font-medium" style={{ color: '#cbd5e1 !important' } as any}>
            ...
            <form onSubmit={handleSearchPlayer} className="flex flex-col sm:flex-row gap-3 max-w-2xl mx-auto">
              <input
                ...
                className="flex-1 bg-slate-900 border-2 border-slate-700 rounded-xl px-5 py-3.5 font-bold placeholder-slate-500 focus:outline-none focus:border-pink-500 text-lg shadow-inner"
                style={{ color: '#ffffff !important' } as any}
              />
              <button
                type="submit"
                disabled={searchLoading}
                className="bg-gradient-to-r from-pink-500 to-purple-600 px-8 py-3.5 rounded-xl font-black hover:opacity-90 shadow-md disabled:opacity-50 text-base tracking-wide"
                style={{ color: '#ffffff !important' } as any}
              >
              ...
              <div className="space-y-6 max-w-2xl mx-auto">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-4 shadow-lg">
                  <div className="border-b border-slate-800 pb-3">
                    {/* FIXED: Pwersahang Puti ang Pangalan */}
                    <h3 className="text-xl font-black flex items-center gap-2 capitalize" style={{ color: '#ffffff !important' } as any}>
                      <User className="w-5 h-5 text-pink-400" /> {playerInfo.full_name}
                    </h3>
                    {/* FIXED: Matingkad na pink para sa ID pill */}
                    <span className="inline-block mt-1 bg-pink-500/20 border border-pink-500/50 px-2.5 py-0.5 rounded-md text-xs font-mono font-bold" style={{ color: '#f472b6 !important' } as any}>
                      ID: {playerInfo.membership_id}
                    </span>
                  </div>

                  {/* FIXED: Pwersahang malinaw na kulay sa details */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    <div className="flex items-center gap-2">
                      <ShieldCheck className="w-4 h-4 text-pink-400" />
                      <span style={{ color: '#cbd5e1 !important' } as any}><strong style={{ color: '#ffffff !important' } as any}>Magulang:</strong> {playerInfo.parent_name}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Phone className="w-4 h-4 text-cyan-400" />
                      <span style={{ color: '#cbd5e1 !important' } as any}><strong style={{ color: '#ffffff !important' } as any}>Contact:</strong> {playerInfo.parent_phone}</span>
                    </div>
                  </div>
                  ...
                  <div className={`p-4 rounded-xl border ${
                    ...
                    <span className="text-xs font-bold tracking-wider uppercase block mb-1" style={{ color: '#94a3b8 !important' } as any}>CURRENT STATUS</span>
                    {historyLogs.length > 0 ? (
                      <div>
                        <div className="text-base font-bold" style={{ color: '#ffffff !important' } as any}>
                          {historyLogs[0].status === 'Active' ? `🟢 Active sa ${historyLogs[0].branch_name}` : `⚪ Checked Out (${historyLogs[0].branch_name})`}
                        </div>
                        <div className="text-xs mt-1 flex items-center gap-1" style={{ color: '#94a3b8 !important' } as any}>
                          <Clock className="w-3.5 h-3.5" /> Huling Update: {new Date(historyLogs[0].checked_in_at).toLocaleString('en-PH')}
                        </div>
                      </div>
                    ) : (
                      <span className="text-sm" style={{ color: '#94a3b8 !important' } as any}>Wala pang naitalang record.</span>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg">
                  {/* FIXED: Title ng History Log ay ginawang malinaw na puti */}
                  <h3 className="text-md font-bold mb-3 flex items-center gap-2" style={{ color: '#ffffff !important' } as any}>📜 History Log</h3>
                  
                  <div className="text-xs font-extrabold uppercase tracking-wider mb-2 border-b border-slate-700 pb-2 grid grid-cols-3">
                    <div style={{ color: '#ffffff !important' } as any}>Branch</div>
                    <div style={{ color: '#ffffff !important' } as any}>Status</div>
                    <div style={{ color: '#ffffff !important' } as any}>Petsa at Oras</div>
                  </div>

                  {historyLogs.length === 0 ? (
                    <p className="text-sm" style={{ color: '#cbd5e1 !important' } as any}>Walang history record.</p>
                  ) : (
                    <div className="space-y-2">
                      {historyLogs.map((log) => (
                        <div key={log.id} className="grid grid-cols-3 items-center py-2.5 border-b border-slate-800/50 text-sm">
                          <div className="font-bold" style={{ color: '#ffffff !important' } as any}>{log.branch_name}</div>
                          <div>
                          ...
                          <div className="font-semibold" style={{ color: '#ffffff !important' } as any}>
        ...
        {activeTab === 'membership' && (
          <div className="space-y-4 max-w-2xl mx-auto text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left pt-2">
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800" style={{ color: '#e2e8f0 !important' } as any}>
                <div className="text-cyan-400 font-bold">Standard Pass</div>
                <p className="text-xs mt-1" style={{ color: '#94a3b8 !important' } as any}>Access sa daily tracking logs at standard branch entries.</p>
              </div>
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800" style={{ color: '#e2e8f0 !important' } as any}>
                <div className="text-purple-400 font-bold">VIP Pass</div>
                <p className="text-xs mt-1" style={{ color: '#94a3b8 !important' } as any}>Priority check-in at exclusive event discounts.</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="max-w-md mx-auto py-4">
            {!isAuthenticated ? (
              <div className="text-center space-y-4">
                ...
                <h2 className="text-xl font-bold text-white" style={{ color: '#ffffff !important' } as any}>Staff Access Only</h2>
                <form onSubmit={handlePinSubmit} className="space-y-3">
                  <input
                    ...
                    className="w-full text-center text-2xl tracking-[10px] py-2.5 bg-slate-900 border border-slate-700 rounded-xl focus:outline-none focus:border-pink-500"
                    style={{ color: '#ffffff !important' } as any}
                  />
                  {pinError && <p className="text-red-400 text-xs font-medium">{pinError}</p>}
                  <button type="submit" className="w-full bg-gradient-to-r from-pink-500 to-purple-600 font-bold py-3 rounded-xl hover:opacity-90 transition-all flex items-center justify-center gap-2 text-sm" style={{ color: '#ffffff !important' } as any}><Unlock className="w-4 h-4" /> Unlock</button>
                </form>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                  ...
                  <button onClick={() => setIsAuthenticated(false)} className="px-3 py-1 bg-slate-800 text-slate-300 rounded-md text-xs font-bold border border-slate-700 flex items-center gap-1" style={{ color: '#ffffff !important' } as any}><Lock className="w-3 h-3" /> Lock</button>
                </div>
                <div className="space-y-2">
                  <label className="block text-xs font-semibold text-slate-400">Select Location:</label>
                  <select value={selectedBranch} onChange={(e) => setSelectedBranch(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-900 border border-slate-700 font-medium text-sm focus:outline-none" style={{ color: '#ffffff !important' } as any}>
                  ...
                  {selectedBranch === 'Others' && (
                    <input type="text" placeholder="Type Branch Name" value={customBranch} onChange={(e) => setCustomBranch(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-900 border border-slate-700 font-medium text-sm focus:outline-none" style={{ color: '#ffffff !important' } as any} />
                  )}
                </div>
                ...
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-3">
                  ...
                  <input type="text" placeholder="e.g. QC-1001" value={staffMembershipId} onChange={(e) => setStaffMembershipId(e.target.value)} className="w-full p-3 bg-slate-950 border border-slate-700 rounded-lg font-bold placeholder-slate-600 focus:outline-none" style={{ color: '#ffffff !important' } as any} />
                  <div className="flex gap-2 text-sm pt-1">
                    <button onClick={handleCheckIn} disabled={staffLoading} className="flex-1 bg-emerald-600 font-bold py-2.5 rounded-lg shadow-md hover:bg-emerald-500" style={{ color: '#ffffff !important' } as any}>🟢 Check In</button>
                    <button onClick={handleCheckOut} disabled={staffLoading} className="flex-1 bg-slate-800 font-bold py-2.5 rounded-lg border border-slate-700 hover:bg-slate-700" style={{ color: '#ffffff !important' } as any}>🔴 Check Out</button>
                  </div>
                </div>
              </div>
            )}
